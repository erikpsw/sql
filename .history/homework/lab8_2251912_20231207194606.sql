...
AS
BEGIN
    DECLARE @_course CHAR(4),@_inserted_course CHAR(4)
    ...
    \SELECT @count = COUNT(*)
    FROM @_inserted_course
    JOIN @_course ON @_inserted_course.column_name = @_course.column_name;


    IF exists (@_inserted_course JOIN @_course)
    BEGIN
        PRINT '不是指定出版社，不能订购!'
        ROLLBACK
    END
...
-- INSERT into sc (snum)
UPDATE sc SET secnum='12001' WHERE snum='s001' and secnum='12601'
